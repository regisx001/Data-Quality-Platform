<script lang="ts">
	...
	// Effective configuration JSON from action response or page load data
	...
	// Parse the stored config JSON into displayable key/value pairs
		...
		<Tabs.Content value="connection" class="space-y-6 mt-4">
					...
					<Card.Content class="p-5 space-y-4">
						<div class="flex items-center gap-4">
							...
							{#if !hasConfig}
								...
								>
									<Info class="size-3.5" />
									No configuration saved yet. Add one in the Configuration
									tab first.
								</span>
							{/if}
						</div>
						...
						{#if isTestingConnection}
							<div
								class="p-3 rounded-lg border border-border bg-muted/30 text-xs space-y-1.5"
							>
								<div
									class="flex items-center gap-1.5 font-medium"
								>
			...
			>
				...
				<Card.Content class="p-5 space-y-5">
					...
					<div>
						<h3
							class="flex items-center gap-1.5 text-xs font-semibold text-muted-foreground mb-2"
						>
