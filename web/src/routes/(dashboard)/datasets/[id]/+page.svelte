		...
		<Tabs.Root value={activeTab} onValueChange={(val) => (activeTab = val)}>
			...
			<Tabs.Content value="rules" class="mt-4 space-y-4">
				<Card.Root class="rounded-xl border-border bg-card shadow-xs">
						...
						<div class="flex items-center justify-between">
							<div>
								...
								<Card.Description
									class="text-xs text-muted-foreground mt-0.5"
								>
									Quality validations configured for columns
									in {dataset.name}.
								</Card.Description>
							</div>
							<Button
								size="sm"
								variant="outline"
								class="h-8 text-xs font-medium cursor-pointer"
							>
								<ShieldCheck
									class="size-3.5 me-1.5 text-primary"
								/>
					...
					<Card.Content
						class="p-6 text-center text-xs text-muted-foreground space-y-2"
					>
						<div
						...
						<p class="max-w-md mx-auto">
							...
							range expectations to validate your data
							automatically.
						</p>
					</Card.Content>
				</Card.Root>
