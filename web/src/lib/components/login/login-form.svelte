...
<div class={cn("grid h-full w-full md:grid-cols-2 lg:grid-cols-[1.1fr_1fr] bg-background overflow-hidden", className)} {...restProps}>
	<!-- Left Side: Appwrite / Vercel Style Architectural Artwork Panel -->
	<div class="bg-black relative hidden md:block h-full w-full border-r border-border overflow-hidden order-1">
		<img
	...
	<div class="flex flex-col justify-between h-full p-6 sm:p-10 md:p-12 lg:p-16 overflow-y-auto order-2">
		<!-- Top: Brand Logo -->
			...
			<form onsubmit={handleSubmit}>
				<Field.Group class="gap-5">
					...
					<Field.Field>
						...
						<Input id="email" type="email" bind:value={email} placeholder="m@example.com" required class="h-10 rounded-xl" />
					</Field.Field>
					...
					<Field.Field>
						<div class="flex items-center justify-between">
							...
							<a href="#/" class="text-xs text-muted-foreground hover:text-foreground underline underline-offset-4">
								Forgot password?
							</a>
						</div>
						<Input id="password" type="password" bind:value={password} required class="h-10 rounded-xl" />
					</Field.Field>
					...
					<!-- Submit Button -->
					<Field.Field>
						<Button type="submit" disabled={isSubmitting} class="w-full h-10 rounded-xl font-medium cursor-pointer">
							{#if isSubmitting}
								Signing in...
							{:else}
								Sign In
					...
					<Field.Field class="grid grid-cols-3 gap-3">
						<Button variant="outline" type="button" class="h-10 rounded-xl cursor-pointer">
					...
					<Field.Description class="text-center pt-2">
						Don't have an account? <a href="/signup" class="text-foreground underline underline-offset-4 hover:text-primary font-medium">Sign up</a>
					</Field.Description>
				</Field.Group>
			</form>
		</div>
