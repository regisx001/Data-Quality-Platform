			...
			<Chart.Container config={chartConfig} class="h-64 w-full">
				<BarChart
					...
					props={{
						...
						xAxis: {
							tickLength: 0,
							format: (d: string, i: number, ticks: any[]) => {
								const totalTicks = ticks?.length || chartData.length || 30;
								const step = Math.max(Math.floor(totalTicks / 5), 1);
								return i % step === 0 || i === totalTicks - 1 ? d : "";
							}
						}
					}}
					legend
